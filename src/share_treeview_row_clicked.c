...
void share_treeview_row_clicked(GtkTreeView * treeview, GdkEventButton * event, gpointer data)
{
    gchar *share;
    GtkTreeIter iter;
    GtkTreePath *path;
    ...
    if( gtk_tree_selection_count_selected_rows(selection) <= 1 )
    {
                ...
                &path, NULL, NULL, NULL) )
        {
            ...
            gtk_tree_model_get(list_store, &iter, 0, &share, -1);

            snprintf(global_share_name, 1000, "%s", (gchar *) share);

            g_free(share);
            gtk_tree_path_free(path);

            populate_share_settings(data);
        }
    }
}
